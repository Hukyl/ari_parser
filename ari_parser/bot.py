...
class Bot(Observer):
    ...
    def subscribe(self, msg):
        ...
        if chat.is_subscribed:
            self._bot.reply_to(msg, "You are already subscribed!")
        else:
            chat.subscribe()
            self._bot.reply_to(msg, "You have subscribed!")

    @_bot.message_handler(commands=['unsubscribe'])
    def unsubscribe(self, msg):
        chat = Chat(msg.chat.id)
        if not chat.is_subscribed:
            self._bot.reply_to(msg, "You are already unsubscribed!")
        else:
            chat.unsubscribe()
            self._bot.reply_to(msg, "You have unsubscribed!")

    def update(self, observable: Updates, attrs: dict, *, additional: dict):
        message = [f"<b>Email</b>: {observable.owner.email}"]
        for k, v in attrs.items():
            message.append(f"<b>{k.title()}</b>: {v}")
        else:
            message = '\n'.join(message)
        for chat in Chat.get_subscribed():
            if additional.get('image'):
                self._bot.send_photo(chat.id, additional['image'], message)
            else:
